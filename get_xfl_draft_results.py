...
def get_xfl_schedule(season=2023, save=False, save_photos=False):
    ...
    for i in json_data['draft']['groups']:
        ...
        print(
            f'Retriving draft results from secton #{draft_section}: {draft_section_name}.')

        for j in tqdm(i['picks']):
            ovr_draft_pick += 1
            row_df = pd.DataFrame({'draft_year': draft_year, 'draft_section': draft_section,
                                  'draft_section_name': draft_section_name}, index=[0])
            ...
            if save_photos == True and player_photo_url != None and player_photo_url != '':
                try:
                    urlretrieve(player_photo_url,
                                filename=f"player_info/photos/{player_id}.png")
                    time.sleep(0.5)
                except:
                    print(
                        f'Could not retrive the photo for player #{player_id}.')
                    time.sleep(1)

    if (save == True):
        main_df.to_csv(f'draft/csv/{season}_xfl_draft.csv', index=False)
        main_df.to_parquet(
            f'draft/parquet/{season}_xfl_draft.parquet', index=False)

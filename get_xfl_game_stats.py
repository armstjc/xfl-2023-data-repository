...
def get_xfl_player_box(game_id:str,save=False):
    ...
    if save == True:
        ...
        main_df.to_csv(f'game_stats/player/csv/{game_id}.csv',index=False)
        main_df.to_parquet(f'game_stats/player/parquet/{game_id}.parquet',index=False)

        with open(f"game_stats/player/json/{game_id}.json", "w+") as f:

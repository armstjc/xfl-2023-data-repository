...
def combine_pbp_files():
    ...
    main_df['FootballFumble_PlayerRecovered'] = main_df['FootballFumble_PlayerRecovered'].replace(
        'TM', -1)
    main_df['FootballFumble_PlayerRecovered'] = main_df['FootballFumble_PlayerRecovered'].fillna(
        0)
    main_df['FootballFumble_PlayerRecovered'] = main_df['FootballFumble_PlayerRecovered'].astype(
        'int')
    main_df['FootballFumble_PlayerRecovered'] = main_df['FootballFumble_PlayerRecovered'].replace(
        0, None)

    main_df['FootballPenalty_PlayerId'] = main_df['FootballPenalty_PlayerId'].replace(
        'TM', -1)
    main_df['FootballPenalty_PlayerId'] = main_df['FootballPenalty_PlayerId'].fillna(
        0)
    main_df['FootballPenalty_PlayerId'] = main_df['FootballPenalty_PlayerId'].astype(
        'int')
    main_df['FootballPenalty_PlayerId'] = main_df['FootballPenalty_PlayerId'].replace(
        0, None)
    ...
    for i in seasons_arr:
        ...
        season_df.to_parquet(
            f"pbp/season/parquet/{i}_xfl_pbp.parquet", index=False)
...
def get_xfl_pbp(game_id: str, save=False, xfl_season=2023):
    ...
    for play in tqdm(json_data):
        count += 1
        row_df = pd.DataFrame({'season': xfl_season, 'game_id': game_id, 'away_team_abv': away_team_abv,
                              'away_team': away_team, 'home_team_abv': home_team_abv, 'home_team': home_team, 'roof': roof}, index=[0])
        ...
        row_df['game_half'] = row_df['qtr'].map(
            {1: 'Half1', 2: 'Half1', 3: 'Half2', 4: 'Half2'})
        ...
        try:
            row_df['quarter_seconds_remaining'] = int(
                play['Properties'][0]['FootballEventContext']['TimeRemSecTotal'])
        except:
            row_df['quarter_seconds_remaining'] = 0

        row_df['half_seconds_remaining'] = row_df['game_half'].apply(lambda x: (((5-int(row_df['qtr']))*900) + (
            row_df['quarter_seconds_remaining']-900) - 1800) if x == 'Half1' else (((5-int(row_df['qtr']))*900) + (row_df['quarter_seconds_remaining']-900)))
        row_df['game_seconds_remaining'] = (
            (5-int(row_df['qtr']))*900) + (row_df['quarter_seconds_remaining']-900)

        row_df['time'] = play['Properties'][0]['FootballEventContext']['TimeRemStr']
        row_df['posteam'] = int(play['Properties'][0]
                                ['FootballEventContext']['PossTeam'])
        ...
        row_df.loc[row_df['posteam'] ==
                   row_df['home_team'], 'home_team_has_pos'] = 1
        row_df.loc[row_df['posteam'] !=
                   row_df['home_team'], 'home_team_has_pos'] = 0

        row_df.loc[row_df['posteam'] == row_df['home_team'],
                   'posteam_abv'] = home_team_abv
        row_df.loc[row_df['posteam'] != row_df['home_team'],
                   'posteam_abv'] = away_team_abv
        row_df.loc[row_df['posteam'] == row_df['home_team'],
                   'defteam_abv'] = away_team_abv
        row_df.loc[row_df['posteam'] != row_df['home_team'],
                   'defteam_abv'] = home_team_abv

        if count == 1:
            receive_1h_ko_team = int(
                play['Properties'][0]['FootballEventContext']['PossTeam'])
            if receive_1h_ko_team == away_team:
                receive_2h_ko_team == away_team
            else:
                receive_2h_ko_team == home_team

        row_df.loc[(row_df['game_half'] == 'Half1') & (
            row_df['posteam'] == receive_2h_ko_team), 'receive_2h_ko_team'] = 1
        row_df.loc[(row_df['game_half'] != 'Half1') | (
            row_df['posteam'] != receive_2h_ko_team), 'receive_2h_ko_team'] = 0
        ...
        row_df.loc[row_df['posteam'] == row_df['home_team'],
                   'posteam_timeouts_remaining'] = row_df['HomeTimeouts']
        row_df.loc[row_df['posteam'] != row_df['home_team'],
                   'posteam_timeouts_remaining'] = row_df['VisTimeouts']
        row_df.loc[row_df['posteam'] == row_df['home_team'],
                   'defteam_timeouts_remaining'] = row_df['HomeTimeouts']
        row_df.loc[row_df['posteam'] != row_df['home_team'],
                   'defteam_timeouts_remaining'] = row_df['VisTimeouts']
        ...
        row_df.loc[(row_df['BallOn_Side'] == 'Home') & (
            row_df['home_team_has_pos'] == 0), 'yardline_100'] = row_df['BallOn_YardNum']
        row_df.loc[(row_df['BallOn_Side'] == 'Home') & (
            row_df['home_team_has_pos'] == 1), 'yardline_100'] = 100 - row_df['BallOn_YardNum']
        row_df.loc[(row_df['BallOn_Side'] == 'Visitor') & (
            row_df['home_team_has_pos'] == 0), 'yardline_100'] = 100 - row_df['BallOn_YardNum']
        row_df.loc[(row_df['BallOn_Side'] == 'Visitor') & (
            row_df['home_team_has_pos'] == 1), 'yardline_100'] = row_df['BallOn_YardNum']
    ...
    try:
        ...
        print(
            'Could not sort dataframe. This may be because [MarkerUTC] does not exist in this JSON, or the dataframe is empty.')

    if len(main_df) > 0:
        try:
            main_df['MapRolePunter'] = main_df['MapRolePunter'].fillna(0)
            main_df['MapRolePunter'] = main_df['MapRolePunter'].astype('int')
            main_df['punt_attempt'] = main_df['MapRolePunter'].apply(
                lambda x: 1 if x > 0 else 0)
            main_df['MapRolePunter'] = main_df['MapRolePunter'].replace(
                0, None)
        except:
            print('Could not find a punter in this game')

        main_df.loc[(main_df['play_type'] != 'EventFootballPass')
                    | (main_df['Result'] != 'TD'), 'pass_touchdown'] = 0
        main_df.loc[(main_df['play_type'] == 'EventFootballPass')
                    & (main_df['Result'] == 'TD'), 'pass_touchdown'] = 1

        main_df.loc[(main_df['play_type'] != 'EventFootballRush')
                    | (main_df['Result'] != 'TD'), 'rush_touchdown'] = 0
        main_df.loc[(main_df['play_type'] == 'EventFootballRush')
                    & (main_df['Result'] == 'TD'), 'rush_touchdown'] = 1

        main_df.loc[(main_df['FootballPlayResult'] == 'ResultKickMade') | (
            main_df['FootballPlayResult'] == 'ResultKickMade'), 'field_goal_attempt'] = 1
        main_df.loc[(main_df['FootballPlayResult'] != 'ResultKickMade') & (
            main_df['FootballPlayResult'] != 'ResultKickMade'), 'field_goal_attempt'] = 0

        main_df.loc[(main_df['FootballPlayResult'] ==
                     'ResultKickMade'), 'field_goal_made'] = 1
        main_df.loc[(main_df['FootballPlayResult'] !=
                     'ResultKickMade'), 'field_goal_made'] = 0
        ...
        main_df['touchdown'] = main_df['pass_touchdown'] + \
            main_df['rush_touchdown']
        ...
        if save == True and len(main_df) > 0:
            ...
            main_df.to_parquet(
                f'pbp/single_game/parquet/{game_id}.parquet', index=False)

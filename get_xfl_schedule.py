...
def get_xfl_schedule(season=2023, save=False):
    ...
    for player in tqdm(json_data):
        ...
        row_df = pd.DataFrame(
            {'Season': xfl_season, 'EventId': official_id}, index=[0])
    ...
    if save == True:
        ...
        main_df.to_parquet(
            f'schedule/{xfl_season}_xfl_schedule.parquet', index=False)

        main_df.to_parquet(
            f'schedule/parquet/{xfl_season}_xfl_schedule.parquet', index=False)
        main_df.to_csv(
            f'schedule/csv/{xfl_season}_xfl_schedule.csv', index=False)

...
def get_xfl_rosters(season=2023, week=1, save=False):
    ...
    if save == True:
        ...
        main_df.to_parquet(
            f'rosters/{xfl_season}_xfl_roster.parquet', index=False)

        main_df['Week'] = xfl_week
        main_df.to_csv(
            f'rosters/weekly_rosters/csv/{int(xfl_season)}_{xfl_week}_xfl_roster.csv', index=False)
        main_df.to_parquet(
            f'rosters/weekly_rosters/parquet/{xfl_season}_{xfl_week}_xfl_roster.parquet', index=False)
